...
class LoraConfig(PeftConfig):
    ...
    lora_init: str = field(default=None, metadata={"help": "Initialization methods"})
    nora_sigma: float = field(default=0.1, metadata={"help": "Nora variance"})
        ...
        for key in key_list:
            ...
            if target_module_found:
                ...
                if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
                    ...
                    new_module = Linear(target.in_features, target.out_features, bias=bias, lora_init=self.peft_config.lora_init, nora_sigma=self.peft_config.nora_sigma, **kwargs)
...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if r > 0:
            ...
            self.nora_sigma = nora_sigma
            # Freezing the pre-trained weight matrix
            self.weight.requires_grad = False
        self.reset_parameters()

        self.lora_init = lora_init
        if self.lora_init == 'sketchy':
            self.sketchy_A_init()
    ...
    @torch.no_grad()
    def sketchy_A_init(self):
        dtype = self.weight.dtype
        if dtype in [torch.float32, torch.float16, torch.bfloat16]:
            weight_tensor = self.weight
        else:
            raise TypeError(f"Unsupported data type for the base layer. Got {dtype}.")
        scale_factor, r = self.scaling, self.r
        # weight_tensor = weight_tensor.to(torch.float32)
        sketch = self.nora_sigma * torch.randn((r, self.out_features)) @ weight_tensor / scale_factor  # (out_feature. r)
        self.lora_A.data = sketch.contiguous()
